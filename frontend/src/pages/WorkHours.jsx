...
import {Clock3,History,Play,Square,CalendarDays,Trash2} from 'lucide-react'
import {useI18n} from '../i18n/I18n'
import {deleteWorkSession,getRunningWorkSession,getWorkSessions,secondsForToday,secondsForWeek,startWorkSession,stopWorkSession} from '../lib/workHoursStore'
...
export default function WorkHours(){
 ...
 async function removeSession(id){
   ...
   try{await deleteWorkSession(id);await load()}catch(e){console.error(e);alert(tx.deleteError)}
 }
 ...
 return <div className="work-hours-page">
   ...
   <section className={`wh-history-card ${historyOpen?'open':''}`}>
     ...
     <div className="wh-history-content">
       {sessions.length?<div className="wh-table-wrap"><table><thead><tr><th>{tx.date}</th><th>{tx.startAt}</th><th>{tx.endAt}</th><th>{tx.duration}</th><th>{tx.actions}</th></tr></thead><tbody>{sessions.slice(0,60).map(s=><tr key={s.id}><td>{formatDate(new Date(s.started_at),lang)}</td><td>{formatTime(new Date(s.started_at),lang)}</td><td>{formatTime(new Date(s.ended_at),lang)}</td><td><b>{formatDuration(Number(s.duration_seconds)||0)}</b></td><td><button type="button" className="wh-delete-session" onClick={()=>removeSession(s.id)} title={tx.delete}><Trash2/><span>{tx.delete}</span></button></td></tr>)}</tbody></table></div>:<div className="wh-empty">{tx.empty}</div>}
     </div>
   </section>
 </div>
}
