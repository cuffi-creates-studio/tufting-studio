...
const RETRO_COLORS=[
 {hex:'#FFC567',name:'Retro Yellow'},
 {hex:'#FB7DA8',name:'Retro Pink'},
 {hex:'#FD5A46',name:'Retro Coral'},
 {hex:'#552CB7',name:'Retro Purple'},
 {hex:'#00995E',name:'Retro Green'},
 {hex:'#058CD7',name:'Retro Blue'}
]
export default function Inventory(){
 ...
 const colorStats=RETRO_COLORS.map(c=>({...c,grams:rows.filter(r=>hex(r.hex)===c.hex).reduce((sum,r)=>sum+num(r.stock_g),0)}))
 ...
 return <div className="business-page inventory retro-business-page">
  <section className="business-shell-card">
   ...
   <div className="biz-layout-split">
    <div className="business-table-wrap"><table className="business-table inventory-table"><thead><tr><th>{tx.yarnType}</th><th>{tx.color}</th><th>{tx.hex}</th><th>{tx.brand}</th><th>{tx.ballG}</th><th>{tx.stock}</th><th>{tx.price}</th><th>{tx.alert}</th><th/></tr></thead><tbody>{shown.map(r=>{const low=num(r.stock_g)<=num(r.min_stock_g);return <tr key={r.id}><td><b>{labelYarn(r.yarn_type,tx)}</b></td><td><div className="inventory-color-cell"><span className="inventory-color-dot" style={{background:hex(r.hex)}}/><span>{r.name||'—'}</span></div></td><td><code>{r.hex||'—'}</code></td><td>{r.brand||'—'}</td><td>{r.ball_g||0} g</td><td><b>{r.stock_g||0} g</b></td><td><b>€{money(r.price_per_ball)}</b></td><td>{low?<span className="biz-stock-alert">{tx.low}</span>:<span className="biz-stock-ok">OK</span>}</td><td><button className="biz-kebab" onClick={()=>open(r)}>•••</button></td></tr>})}</tbody></table>{!shown.length&&<div className="biz-empty">{tx.empty}</div>}</div>
    <aside className="biz-summary colorful-summary inventory-summary"><h3>{tx.summary}</h3><div className={`biz-donut inventory-color-donut ${totalG===0?'is-empty':''}`} style={{background:inventoryGradient(colorStats,totalG)}}><div className="biz-donut-center"><b>{tx.total}</b><strong>{totalG.toLocaleString()} g</strong></div></div><div className="biz-legend inventory-color-legend">{colorStats.map(c=><div className="biz-legend-row" key={c.hex}><span className="inventory-legend-swatch" style={{background:c.hex}}/><span>{c.hex}</span><b>{c.grams.toLocaleString()} g</b></div>)}</div>{totalG===0&&<div className="inventory-no-data">{tx.noStock}</div>}<div className="biz-summary-total"><span>{tx.totalValue}</span><strong>€{money(totalValue)}</strong></div></aside>
   </div>
  </section>
  ...
  <dialog className="biz-dialog retro-dialog inventory-dialog" ref={dialog} onClose={()=>setEditing(null)}>
   ...
   <form className="biz-form retro-form" onSubmit={submit}>
    ...
    <section className="retro-field-card coral color-card"><div className="retro-field-title"><Palette/>{tx.colorCard}</div><div className="retro-catalog-swatches">{RETRO_COLORS.map(c=><button type="button" key={c.hex} className={hex(form.hex)===c.hex?'active':''} onClick={()=>setForm(v=>({...v,hex:c.hex}))} title={`${c.name} ${c.hex}`}><span style={{background:c.hex}}/><small>{c.hex}</small></button>)}</div></section>
...
function inventoryGradient(stats,total){
 if(!total)return'conic-gradient(#E9E1D5 0 100%)'
 let at=0
 const parts=stats.filter(c=>c.grams>0).map(c=>{const next=at+(c.grams/total)*100;const part=`${c.hex} ${at}% ${next}%`;at=next;return part})
