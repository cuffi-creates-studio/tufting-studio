...
const RETRO_PALETTE=[
 {hex:'#FFC567',name:'Mustard'},
 {hex:'#FB7DA8',name:'Rozë'},
 {hex:'#FD5A46',name:'Korall'},
 {hex:'#552CB7',name:'Vjollcë'},
 {hex:'#00995E',name:'Jeshile'},
 {hex:'#058CD7',name:'Blu'},
]
  ...
  <section className="business-shell-card inventory-shell-v3">
   ...
   <div className="biz-layout-split inventory-layout-v3">
    ...
    <aside className="biz-summary colorful-summary inventory-summary-v3">
     <h3>{tx.summary}</h3>
     <div className={`inventory-real-donut ${rows.length?'has-data':'is-empty'}`} style={rows.length?{background:donut}:undefined}>
      <div className="inventory-real-donut-center"><small>{tx.total}</small><strong>{totalG.toLocaleString()} g</strong>{!rows.length&&<span>{tx.noMaterials}</span>}</div>
     </div>
     {rows.length>0&&<div className="biz-legend inventory-legend-v3">{rows.slice(0,8).map(r=><div className="biz-legend-row" key={r.id}><YarnBall hex={r.hex} size="xs"/><span>{r.name}</span><b>{num(r.stock_g).toLocaleString()} g</b></div>)}</div>}
  ...
  <dialog className="biz-dialog retro-dialog inventory-dialog inventory-dialog-v3" ref={dialog} onClose={()=>setEditing(null)}>
   ...
   <form className="biz-form retro-form inventory-form-v3" onSubmit={submit}>
    ...
    <section className="retro-field-card coral color-card inventory-palette-card"><div className="retro-field-title"><Palette/>{tx.colorCard}</div>
     <div className="inventory-palette-layout"><YarnBall hex={form.hex} size="xl"/><div className="inventory-retro-palette">{RETRO_PALETTE.map(c=><button type="button" key={c.hex} title={`${c.name} ${c.hex}`} className={hex(form.hex)===c.hex?'selected':''} style={{'--swatch':c.hex}} onClick={()=>setForm(v=>({...v,hex:c.hex}))}><i/><span>{c.hex}</span></button>)}</div></div>
...
function labels(lang){
 if(lang==='de')return{title:'Inventar',subtitle:'Verwalte Garnbestand, Farben, Marken und echte Preise.',export:'Export',search:'Material suchen...',add:'Material hinzufügen',yarnType:'Garnart',acrylic:'Acryl',wool:'Wolle',color:'Name / Farbe',hex:'HEX-Code',brand:'Marke',ballG:'Ballgewicht',stock:'Gesamtbestand (g)',price:'Preis / Ball',alert:'Bestand',low:'Niedrig',summary:'Bestandsübersicht',total:'Gesamt',totalValue:'Inventarwert',colorCard:'Retro-Farbpalette',min:'Mindestbestand (g)',save:'Material speichern',saving:'Speichern…',cancel:'Schließen',edit:'Material bearbeiten',delete:'Löschen',empty:'Noch keine Materialien.',emptyHint:'Füge dein erstes Material hinzu, um den echten Bestand zu sehen.',noMaterials:'Keine Materialien',confirmDelete:'Dieses Material löschen?',dialogSubtitle:'Erfasse reales Garn und Bestand.',saved:'Gespeicherte PDFs',pdfTitle:'Tufting Studio – Inventar',yarnHelp:'Wähle Acryl oder Wolle.',brandPlaceholder:'z.B. Kartopu, Alize…',brandHelp:'Marke des Garns.',colorPlaceholder:'z.B. Retro Rosa',colorHelp:'Name der Farbe.',ballHelp:'Gewicht eines Balls.',paletteHelp:'Wähle eine Retro-Farbe oder trage unten einen HEX-Code ein.',stockHelp:'Gesamte Gramm dieser Farbe im Lager.',hexHelp:'Exakter HEX-Code der Farbe.',priceHelp:'Preis eines Balls in der gewählten Grammatur.',minHelp:'Warnschwelle für niedrigen Bestand.'}
 if(lang==='en')return{title:'Inventory',subtitle:'Manage yarn stock, colors, brands and real prices.',export:'Export',search:'Search material...',add:'Add material',yarnType:'Yarn type',acrylic:'Acrylic',wool:'Wool',color:'Name / Color',hex:'HEX code',brand:'Brand',ballG:'Ball weight',stock:'Total stock (g)',price:'Price / ball',alert:'Stock',low:'Low stock',summary:'Stock summary',total:'Total',totalValue:'Inventory value',colorCard:'Retro color palette',min:'Minimum stock (g)',save:'Save material',saving:'Saving…',cancel:'Close',edit:'Edit material',delete:'Delete',empty:'No inventory items yet.',emptyHint:'Add your first material to start real stock statistics.',noMaterials:'No materials',confirmDelete:'Delete this material?',dialogSubtitle:'Record real yarn and stock data.',saved:'Saved PDFs',pdfTitle:'Tufting Studio – Inventory',yarnHelp:'Choose Acrylic or Wool.',brandPlaceholder:'e.g. Kartopu, Alize…',brandHelp:'Yarn brand.',colorPlaceholder:'e.g. Retro Pink',colorHelp:'Name of the color.',ballHelp:'Weight of one yarn ball.',paletteHelp:'Choose a retro color or enter a HEX code below.',stockHelp:'Total grams available for this color.',hexHelp:'Exact HEX color code.',priceHelp:'Price of one ball at the selected weight.',minHelp:'Low-stock warning threshold.'}
 return{title:'Inventari',subtitle:'Menaxho leshin, ngjyrat, markat dhe çmimet reale.',export:'Eksporto',search:'Kërko material...',add:'Shto material',yarnType:'Lloji i leshit',acrylic:'Akryl',wool:'Lesh',color:'Emri / Ngjyra',hex:'Kodi HEX',brand:'Marka',ballG:'Gramatura e topit',stock:'Stoku total (g)',price:'Çmimi / top',alert:'Alarm stok',low:'Stok i ulët',summary:'Përmbledhje stoku',total:'Totali',totalValue:'Vlera totale e inventarit',colorCard:'Paleta Retro',min:'Minimumi i stokut (g)',save:'Ruaj materialin',saving:'Po ruhet…',cancel:'Mbyll',edit:'Redakto materialin',delete:'Fshi',empty:'Ende nuk ka materiale.',emptyHint:'Shto materialin e parë që statistikat të dalin nga stoku real.',noMaterials:'Nuk ka materiale',confirmDelete:'Ta fshij këtë material?',dialogSubtitle:'Shto një material të ri me të dhëna reale.',saved:'PDF / Faturat e ruajtura',pdfTitle:'Tufting Studio – Inventari',yarnHelp:'Zgjidh Akryl ose Lesh.',brandPlaceholder:'p.sh. Kartopu, Alize…',brandHelp:'Shkruaj markën e leshit.',colorPlaceholder:'p.sh. Rozë Retro',colorHelp:'Shkruaj emrin e ngjyrës.',ballHelp:'Pesha e një topi leshi.',paletteHelp:'Zgjidh një nga ngjyrat retro ose shkruaj kodin HEX më poshtë.',stockHelp:'Sa gram ke gjithsej nga kjo ngjyrë.',hexHelp:'Kodi i saktë HEX i ngjyrës.',priceHelp:'Çmimi i një topi sipas gramaturës së zgjedhur.',minHelp:'Kur stoku bie në këtë nivel, del alarm.'}
}
